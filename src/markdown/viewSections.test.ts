...
import { buildViewSections, findUnchangedSectionForRow } from "./viewSections";
...
describe("buildViewSections", () => {
  ...
  it("interleaves changed and unchanged runs", () => {
    const rows = alignMarkdown("# Same\n\nOld para\n\n# Tail\n", "# Same\n\nNew para\n\n# Tail\n");
    const sections = buildViewSections(rows);
    expect(sections.map((s) => s.kind)).toEqual(["unchanged", "changed", "unchanged"]);
  ...
  it("finds the unchanged section that owns a row", () => {
    ...
    expect(findUnchangedSectionForRow(sections, firstEqual.id)?.id).toBe("unchanged-0");
