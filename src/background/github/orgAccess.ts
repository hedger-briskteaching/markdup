 ...
 */
export async function setAccessWarning(warning: AccessWarning | null): Promise<void> {
  if (!warning) {
 ...
 */
export async function probeOauthOrgAccess(token: string): Promise<AccessWarning | null> {
  ...
  try {
    ...
    if (error instanceof GitHubApiError && isOrgOauthRestrictionError(error.message)) {
  ...
  for (const org of orgs.slice(0, 15)) {
    try {
      await githubFetch(`/orgs/${encodeURIComponent(org.login)}/repos?per_page=1&type=all`, {
        token,
      });
    } catch (error) {
      if (error instanceof GitHubApiError && isOrgOauthRestrictionError(error.message)) {
 ...
 */
export async function recordApiAccessWarning(errorMessage: string): Promise<AccessWarning | null> {
