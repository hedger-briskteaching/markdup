...
describe("device flow", () => {
  ...
  it("polls until access_token is returned", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(Response.json({ access_token: "gho_test_token" }));
  ...
  it("increases interval on slow_down then succeeds", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(Response.json({ access_token: "gho_slow_ok" }));
...
describe("setPersonalAccessToken", () => {
  ...
  it("stores a valid classic PAT", async () => {
    const fetchImpl = vi.fn(async () => Response.json({ login: "hedger" }));
  ...
  it("rejects tokens that do not look like PATs", async () => {
    await expect(setPersonalAccessToken("not-a-token", fetch as typeof fetch)).rejects.toThrow(
      /does not look like a GitHub token/i,
    );
  });
