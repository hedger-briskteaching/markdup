import { GITHUB_OAUTH_CLIENT_ID, GITHUB_OAUTH_SCOPES } from "../../shared/githubAuth";
...
export async function clearAccessToken(): Promise<void> {
  cancelDevicePoll();
  await chrome.storage.local.remove([TOKEN_KEY, LOGIN_KEY, OBTAINED_KEY, METHOD_KEY]);
}
 ...
 */
export async function startDeviceFlow(fetchImpl: FetchLike = fetch): Promise<DeviceCodeResponse> {
...
export async function pollDeviceFlow(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    const data = (await response.json()) as AccessTokenSuccess & AccessTokenPending;
