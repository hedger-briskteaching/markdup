...
describe("findRowIdForThread", () => {
  ...
  it("matches a LEFT thread to the Before block", () => {
    ...
    const id = findRowIdForThread(rows, thread({ rootId: 2, side: "LEFT", startLine: 1, line: 1 }));
    expect(id).toBeTruthy();
  });
});
...
describe("isReanchored", () => {
  it("detects when originalLine differs from current line", () => {
    expect(
      isReanchored(
        thread({ rootId: 1, side: "RIGHT", startLine: 1223, line: 1223 }, { originalLine: 1222 }),
      ),
    ).toBe(true);
  });
  ...
  it("is false when original matches current", () => {
    expect(
      isReanchored(
        thread({ rootId: 1, side: "RIGHT", startLine: 3, line: 3 }, { originalLine: 3 }),
      ),
    ).toBe(false);
  });
});
...
function tableThreadHost(lines: { startLine: number; line: number }): HTMLElement {
...
const LIST_MD = ["# Plan", "", "- item 1", "- item 2", "- item 3", ""].join("\n");

/**
 * Nested list. Outer items on lines 3 and 6, nested items on lines 4 and 5.
 */
const NESTED_LIST_MD = ["# Plan", "", "- item 1", "  - sub a", "  - sub b", "- item 2", ""].join(
  "\n",
);
...
function listChildren(host: HTMLElement): Element[] {
  return [...host.querySelectorAll('.rgm-rich-cell[data-side="RIGHT"] ul > *')];
}
...
describe("renderThreadCards", () => {
  it("renders an Open chip under the matching row", () => {
    ...
    const card = host.querySelector('.rgm-rich-cell[data-side="RIGHT"] [data-rgm-thread-card]');
  ...
  it("mounts a table thread in its own row under the row it targets", () => {
    ...
    const rows = [...host.querySelectorAll('.rgm-rich-cell[data-side="RIGHT"] tr')];
    const cardRow = host.querySelector('.rgm-rich-cell[data-side="RIGHT"] .rgm-rich-comment-row');
  ...
  it("spans the comment row across every table column", () => {
    const host = tableThreadHost({ startLine: 6, line: 6 });
    const holder = host.querySelector<HTMLTableCellElement>(".rgm-rich-comment-row > td");
  ...
  it("mounts inside the nested list for a thread on a nested item", () => {
    ...
    const slot = host.querySelector(".rgm-rich-comment-item");
    const nested = [...host.querySelectorAll('.rgm-rich-cell[data-side="RIGHT"] ul ul > *')];
  ...
  it("renders every comment in the thread as Markdown", () => {
    ...
    const actions = [...host.querySelectorAll(".rgm-thread-comment")].map((el) => el.textContent);
    expect(actions.some((t) => t?.includes("Edit") && t.includes("Delete"))).toBe(true);
  });
