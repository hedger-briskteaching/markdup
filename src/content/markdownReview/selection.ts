 ...
 * @returns Nothing.
 */
export function setRichViewContext(richRoot: Element, ctx: RichViewContext): void {
 ...
 */
export function getRichViewContext(richRoot: Element): RichViewContext | undefined {
 ...
 */
export function isUnchangedSectionExpanded(richRoot: Element, sectionId: string): boolean {
 ...
 * @returns Nothing.
 */
export function expandUnchangedSection(richRoot: Element, sectionId: string): void {
 ...
 * @returns Nothing.
 */
export function collapseUnchangedSection(richRoot: Element, sectionId: string): void {
  const ctx = getRichViewContext(richRoot);
  if (!ctx) return;
  const next = new Set(ctx.expandedUnchangedIds ?? []);
 ...
 */
export function toggleUnchangedSection(richRoot: Element, sectionId: string): boolean {
...
export function selectionToSourceRange(
  ...
  const start = anchorForPoint(startCell, range.startContainer, range.startOffset, ctx);
  ...
  if (start.host === end.host) {
    ...
    mapped = offsetsToSourceRange(start.anchor, startOffset, endOffset, quotedText);
  } else {
 ...
 * @returns Nothing.
 */
export function applySelectionHighlight(richRoot: HTMLElement, sourceRange: SourceRange): void {
 ...
 * @returns Nothing.
 */
export function applyThreadHoverHighlight(richRoot: HTMLElement, sourceRange: SourceRange): void {
  clearThreadHoverHighlight(richRoot);
  const layer = createHighlightLayer(THREAD_HOVER_ATTR, "rgm-thread-hover-layer");
  appendRangeBoxes(richRoot, layer, sourceRange, {
 ...
 * @returns Nothing.
 */
export function paintCommentedMarks(richRoot: HTMLElement, marks: CommentedMark[]): void {
...
function paintTargetsIn(content: Element, domRange: Range): Element[] {
  const subs = [...content.querySelectorAll<HTMLElement>(SUB_ANCHOR_SELECTOR)].filter((el) =>
    domRange.intersectsNode(el),
  );
  // Nested items (a list inside a list item) would paint twice over.
  const outermost = subs.filter((el) => !subs.some((other) => other !== el && other.contains(el)));
 ...
 */
export function findCellForSourceRange(richRoot: Element, range: SourceRange): HTMLElement | null {
...
export function findSubAnchorForSourceRange(
  ...
  const overlapping = subAnchorsIn(cell).filter((el) => elementOverlapsRange(el, range));
 ...
 */
export function domRangeForSourceRange(richRoot: Element, sourceRange: SourceRange): Range | null {
  const ctx = contexts.get(richRoot);
  if (!ctx) return null;
...
function rangeHostInCell(
  ...
  if (side) {
    const overlapping = subAnchorsIn(cell).filter((el) => elementOverlapsRange(el, range));
    const pick = which === "first" ? overlapping[0] : overlapping[overlapping.length - 1];
 ...
 */
function cellsOverlappingRange(richRoot: Element, range: SourceRange): HTMLElement[] {
  const cells = [
 ...
 */
function subAnchorForPoint(subs: HTMLElement[], node: Node, offset: number): HTMLElement | null {
  if (subs.length === 0) return null;

  const point =
    node instanceof Element ? (node.childNodes[offset] ?? node.lastChild ?? node) : node;
...
function anchorForPoint(
  ...
  if (sub && subAnchor) {
    ...
    return {
      ...
      offsetAt: (n, o) => walkTextOffset(sub, n, o) ?? (nodePrecedesElement(n, sub) ? 0 : length),
    };
  }
 ...
 */
function anchorFromCell(cell: HTMLElement, ctx: RichViewContext): BlockSourceAnchor | null {
  const side = sideFromCell(cell);
 ...
 */
export function srcOffsetInCell(cell: HTMLElement, node: Node, offset: number): number | null {
  const content = cell.querySelector(".rgm-rich-content");
 ...
 */
function closestSrcOffsetHost(node: Node, content: Element): HTMLElement | null {
 ...
 */
function textOffsetWithin(host: Element, target: Node, targetOffset: number): number {
  if (target === host) {
 ...
 */
function walkTextOffset(root: Element, target: Node, targetOffset: number): number | null {
  if (target === root) {
    let total = 0;
