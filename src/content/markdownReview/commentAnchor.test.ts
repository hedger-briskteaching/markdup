 ...
 */
function richRootWithThread(threadId: number, commentIds: number[]): HTMLElement {
...
describe("followCommentHash", () => {
  ...
  it("waits for the thread cards to render", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        root.querySelector("[data-rgm-thread-card]")?.classList.contains("rgm-thread-card-flash"),
      ).toBe(true);
    });
  });
