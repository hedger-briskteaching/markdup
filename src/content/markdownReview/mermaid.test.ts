...
describe("Mermaid rich rendering", () => {
  ...
  it("creates an extension frame and selectable source fallback", () => {
    ...
    expect(figure.querySelector("iframe")?.title).toBe("Interactive Mermaid diagram");
    expect(figure.querySelector("details summary")?.textContent).toBe("Diagram source");
    expect(figure.querySelector('[data-src-offset="0"]')?.textContent).toContain("A --> B");
