...
describe("rich diff layout selector", () => {
  ...
  it("changes layout without rebuilding the rich columns", () => {
    ...
    const before = root.querySelector<HTMLElement>('.rgm-rich-cell[data-side="LEFT"]')!;
    const after = root.querySelector<HTMLElement>('.rgm-rich-cell[data-side="RIGHT"]')!;
    ...
    expect(
      root.querySelector('[data-rgm-layout-option="before"]')?.getAttribute("aria-pressed"),
    ).toBe("true");
    expect(
      root.querySelector('[data-rgm-layout-option="split"]')?.getAttribute("aria-pressed"),
    ).toBe("false");
  });
  ...
  it("labels each option with data-tooltip rather than the slow title", () => {
    ...
    const buttons = [...region.querySelectorAll<HTMLButtonElement>("[data-rgm-layout-option]")];
  ...
  it("restores the selected layout when the rich view remounts", () => {
    ...
    expect(
      remounted.querySelector('[data-rgm-layout-option="after"]')?.getAttribute("aria-pressed"),
    ).toBe("true");
  });
});
