...
describe("mountCommentEditor", () => {
  ...
  it("omits Link, Paragraph, and Italic toolbar controls", () => {
    ...
    const labels = [...host.querySelectorAll<HTMLButtonElement>(".rgm-comment-editor-btn")].map(
  ...
  it("exposes a visible data-tooltip on each toolbar button", () => {
    ...
    const buttons = [...host.querySelectorAll<HTMLButtonElement>(".rgm-comment-editor-btn")];
  ...
  it("toggles a bulleted list on and off", () => {
    ...
    const bulletBtn = [...host.querySelectorAll<HTMLButtonElement>(".rgm-comment-editor-btn")].find(
      (b) => (b.getAttribute("aria-label") ?? "").includes("Bulleted list"),
    )!;
  ...
  it("highlights bold and inline code when the whole selection has the mark", () => {
    ...
    view.dispatch(
      view.state.tr.setSelection(TextSelection.create(view.state.doc, worldFrom, worldTo)),
    );
    ...
    view.dispatch(
      view.state.tr.setSelection(TextSelection.create(view.state.doc, codeFrom, codeTo)),
    );
    ...
    view.dispatch(
      view.state.tr.setSelection(TextSelection.create(view.state.doc, worldFrom, worldTo + 2)),
    );
    expect(boldBtn.classList.contains("is-active")).toBe(false);
  });
