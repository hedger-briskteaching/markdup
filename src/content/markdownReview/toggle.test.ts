...
describe("injectToggle", () => {
  ...
  it("hides native File view and places Markdup after native controls with a divider", () => {
    ...
    const divider = region.querySelector<HTMLElement>("[data-rgm-header-divider]");
    const switchBtn = toggle?.querySelector<HTMLButtonElement>('[role="switch"]');
    const kebab = region.querySelector("button .octicon-kebab-horizontal")?.closest("button");
  ...
  it("shows the toggle NUX once on first mount", async () => {
    ...
    expect(region.textContent).toContain("review markdown files in rich text format");
  ...
  it("asks the background worker to open Settings from the auth panel", async () => {
    ...
    const switchBtn = region.querySelector<HTMLButtonElement>('[data-rgm-toggle] [role="switch"]')!;
    switchBtn.click();
    ...
    const buttons = [...region.querySelectorAll<HTMLButtonElement>("[data-rgm-auth-panel] button")];
    ...
    expect(sendMessage.mock.calls.some((call) => call[0]?.type === "OPEN_OPTIONS")).toBe(true);
  });
});
