...
export function disposeMermaidDiagrams(root: ParentNode): void {
  const ownFigure = root instanceof HTMLElement && root.matches("[data-rgm-mermaid]") ? [root] : [];
  for (const figure of [
    ...ownFigure,
...
function mermaidFrameUrl(): string {
  return globalThis.chrome?.runtime?.getURL?.(MERMAID_FRAME_PATH) ?? MERMAID_FRAME_PATH;
}

function postRenderRequest(frame: HTMLIFrameElement, requestId: string, source: string): void {
...
function createRequestId(): string {
  return globalThis.crypto?.randomUUID?.() ?? `mermaid-${Date.now()}-${Math.random()}`;
}
