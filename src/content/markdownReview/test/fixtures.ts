...
export function createFileRegion(options: FileRegionOptions): HTMLElement {
  ...
  if (fileExpanded !== undefined) {
    ...
    icon.classList.add("octicon", fileExpanded ? "octicon-chevron-down" : "octicon-chevron-right");
...
export function setFileCollapsed(region: Element, collapsed: boolean): void {
  ...
  if (fileHeader) {
    fileHeader.classList.toggle("DiffFileHeader-module__collapsed__ZY5uc", collapsed);
  }

  const icon = region.querySelector("svg.octicon-chevron-down, svg.octicon-chevron-right");
  if (icon) {
  ...
  if (tip && (tip.textContent === "Collapse file" || tip.textContent === "Expand file")) {
    tip.textContent = collapsed ? "Expand file" : "Collapse file";
  }

  if (collapsed) {
    region.querySelector("div.border.position-relative.rounded-bottom-2")?.remove();
  }
}
