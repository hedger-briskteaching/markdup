...
function selectNeedle(root: Element, needle: string): void {
  const contents = [...root.querySelectorAll(".rgm-rich-cell-after .rgm-rich-content")];
...
describe("bindComposer", () => {
  ...
  it("keeps the highlight on collapsed-selection clicks while the composer is open", async () => {
    ...
    host
      .querySelector<HTMLButtonElement>("[data-rgm-comment-bubble] .rgm-comment-bubble-btn")!
      .click();
    await vi.waitFor(() => {
  ...
  it("opens the composer from the Comment bubble and posts", async () => {
    ...
    expect(sendMessage.mock.calls.some((call) => call[0]?.type === "FETCH_THREAD_INDEX")).toBe(
      true,
    );
  });
  ...
  it("reloads the page when leaving rich mode after posting a comment", () => {
    ...
    expect(document.documentElement.hasAttribute("data-rgm-comments-dirty")).toBe(false);
  });
});
