...
describe("thread reply / edit / delete", () => {
  it("posts a reply and syncs", async () => {
    ...
    const editorHost = host.querySelector<HTMLElement>(".rgm-thread-reply .rgm-comment-editor")!;
    getCommentEditorForTest(editorHost)!.setMarkdown("A reply");
    host.querySelector<HTMLButtonElement>(".rgm-thread-reply .rgm-composer-btn-primary")!.click();
  ...
  it("edits an own comment and syncs", async () => {
    const sendMessage = vi
      .fn()
      .mockImplementation(async (message: { type: string; body?: string }) => {
    ...
    const editorHost = host.querySelector<HTMLElement>(".rgm-thread-edit .rgm-comment-editor")!;
    getCommentEditorForTest(editorHost)!.setMarkdown("Updated body");
    host.querySelector<HTMLButtonElement>(".rgm-thread-edit .rgm-composer-btn-primary")!.click();
...
describe("stale page state after a mutation", () => {
  ...
  function deleteWith(fetchIndex: () => unknown): {
    host: HTMLElement;
  ...
  it("flags the page once the delete lands", async () => {
    ...
    expect(document.documentElement.hasAttribute("data-rgm-comments-dirty")).toBe(true);
  });
