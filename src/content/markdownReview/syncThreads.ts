...
import { getRichViewContext, setRichViewContext, type RichViewContext } from "./selection";
 ...
 * @returns Nothing.
 */
export function applyServerThreads(richRoot: Element, threads: ReviewThreadDto[]): void {
 ...
 */
function threadsContainComment(threads: ReviewThreadDto[], commentId: number): boolean {
  return threads.some(
    (thread) => thread.rootId === commentId || thread.comments.some((c) => c.id === commentId),
  );
}
...
async function fetchThreadIndex(
  ...
  try {
    ...
    return {
      error: error instanceof Error ? error.message : "Failed to load review comments.",
    };
  }
}
